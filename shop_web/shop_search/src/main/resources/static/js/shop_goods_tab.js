...
function shop_goods_easytabs(menunr, active) {
    ...
    document.getElementById(tablink_idname[menunr] + active).className = 'tab' + active + ' ' + hoverClassName;
...
function start_autochange() {
    counter = counter + 1;
    timer = setTimeout("start_autochange()", IntervalTime);
    if (counter == changespeed + 1) {
        currenttab++;
        if (currenttab > totaltabs) {
            currenttab = 1
        }
        shop_goods_easytabs(autochangemenu, currenttab);
        restart_autochange();
    }
}
...
window.onload = function () {
    ...
    var b = 1;
    do {
        shop_goods_easytabs(b, loadtabs[a]);
        a++;
        b++;
    } while (b <= menucount);
    if (autochangemenu != 0) {
